...
class EntityEdit extends Component{
  ...
  async componentDidMount(){
    if(this.props.match.params.id !== 'new') {
      const entity = await(await fetch(`/${this.props.match.params.entityType}/${this.props.match.params.id}`)).json();
  ...
  render(){
    const {item} = this.state;
    const title = <h2>{item.id == "new" ? 'Add Entity':'Edit Entity'}</h2>
    return <div>
    <AppNavbar />
    <Container>
      {title}
